...
class Queue:
    ...
    def checkRep(self):
        ...
        assert self.tail-self.head == self.size or (self.tail-self.head == 0 and self.size==self.max)  if self.tail>=self.head else self.tail+self.max-self.head==self.size
        return
